...
# p12 用随机强密码加密（保存在仅本人可读的文件里，CI Secret 也用它）
P12_PWD=$(/usr/bin/openssl rand -base64 24)
...
/usr/bin/openssl pkcs12 -export -inkey "$TMP/key.pem" -in "$TMP/cert.pem" \
    -out "$TMP/cert.p12" -passout "pass:$P12_PWD" >/dev/null 2>&1

security import "$TMP/cert.p12" -k "$HOME/Library/Keychains/login.keychain-db" \
    -P "$P12_PWD" -T /usr/bin/codesign >/dev/null
...
printf '%s' "$P12_PWD" > "$KEEP_DIR/dev-cert-password"
chmod 600 "$KEEP_DIR/dev-cert-password"
...
echo "   gh secret set HUNK_CERT_PWD -b \"$(cat "$KEEP_DIR/dev-cert-password")\""
